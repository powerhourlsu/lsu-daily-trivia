...
function rowsToPuzzles(rows) {
  ...
  return dataRows.map(cols => {
    ...
    return {
      date: (date || "").trim(), answer: (answer || "").trim(),
...
export async function getTodaysPuzzle() {
  ...
  if (!url || url.includes("PASTE-YOUR-ID-HERE")) return { error: "No sheet connected yet." };
  let text;
  try {
    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) throw new Error(`Sheet fetch failed: ${res.status}`);
    text = await res.text();
  } catch (err) { return { error: `Could not load puzzles: ${err.message}` }; }
  const puzzles = rowsToPuzzles(parseCSV(text)).filter(p => p.date && p.answer && p.clues.length === 5);
  if (puzzles.length === 0) return { error: "No valid rows in sheet yet." };
