...
import { Lock, ChevronRight, RotateCcw, Trophy, Check, Flame, Timer, Share2 } from "lucide-react";
...
function isCorrectGuess(guess, answer) {
  ...
  const parts = answer
    .split(" ")
    .filter(p => !/^(jr\.?|sr\.?|i{1,3}|iv)$/i.test(p.trim()));
  const lastName = parts[parts.length - 1];
  return g === normalize(lastName);
}
...
export default function TriviaGame({ puzzle }) {
  ...
  function handleTimeout() {
    ...
    setClueIndex(i => {
      if (i + 1 >= MAX_CLUES) { setStatus("lost"); return i; }
      return i + 1;
    });
  }
    ...
    <main style={{ background: "#F4EFE3", minHeight: "100vh", fontFamily: "'Inter', system-ui, sans-serif", color: "#241433", padding: "26px 16px 40px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <div style={{ background: "#461D7C", borderRadius: "14px 14px 0 0", padding: "18px 20px 22px", color: "#FDD023" }}>
          ...
          <h1 className="pf" style={{ fontSize: 28, margin: "4px 0 0", color: "#FDD023" }}>Who Am I?</h1>
          <div style={{ fontSize: 13, color: "#E8DCF5", marginTop: 2 }}>5 clues. Fewer clues, higher score.</div>
        </div>

        {/* Perforation */}
        <div style={{ height: 0, borderTop: "2px dashed #C9BFA8", background: "#F4EFE3", position: "relative" }}>
          {[...Array(14)].map((_, i) => (
        ...
        <div style={{ background: "#FFFDF8", border: "1px solid #E3DBC8", borderTop: "none", borderRadius: "0 0 14px 14px", padding: "20px 20px 24px", boxShadow: "0 6px 18px rgba(70,29,124,0.08)" }}>
          ...
          {/* Timer */}
          {status === "playing" && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, color: secondsLeft <= 15 ? "#C23B3B" : "#241433", fontSize: 13 }}>
              <Timer size={14} />
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
            {resultSquares.map((s, i) => (
              <div key={i} style={{ flex: 1, height: 8, borderRadius: 4, background: s === "win" ? "#FDD023" : s === "used" ? "#461D7C" : "#E3DBC8" }} />
            ))}
          </div>
          ...
          {status === "playing" ? (
            ...
            <div style={{ textAlign: "center", padding: "10px 4px 2px" }}>
              ...
              {/* Fun facts with football emojis */}
          ...
          {status === "playing" && (
            <button onClick={giveUp} style={{ marginTop: 10, background: "transparent", border: "none", color: "#9A8E70", fontSize: 12.5, textDecoration: "underline", cursor: "pointer", padding: 0 }}>
